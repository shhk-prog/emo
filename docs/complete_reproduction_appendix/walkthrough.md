...
6. **E.16 Mood Congruency 正本値と位置づけ**:
...
7. **Appendix D (v2) のトーン抑制と事実限定**:
   - 推論や過度な断定を排し、Phase 1〜9 の実測成果物から直接確認できる事実のみに整理。
8. **Appendix G の実在ファイル厳選**:
   - 実在が確認されたスクリプトおよび結果CSVのみを網羅掲載。

---

## 3. 自動検証結果
全19項目の自動テストスクリプトを実行し、すべての正本値が合致（`ALL 19 VERIFIED CHECKS PASSED`）していることを確認しました。
これで査読者からの指摘リスクを極限まで排除した、正本に完全準拠した Appendix が完成しました。
